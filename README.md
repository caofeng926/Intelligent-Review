...
| 表 | 行数 | 说明 |
|---|---:|---|
| `batches` | 18 | 批次 (含 NHSA 33 条 + 2025 PDF 45 条) |
| `rules` | 78 | 审核规则 |
| `knowledge_points` | 21,886 | 知识点 (药品 / 项目) |
| `knowledge_point_codes` | 30,829 | 知识点 ↔ 医保编码 (多对一) |
...
**目标服务器**:`132.232.152.250:5000` (Ubuntu 22.04+)
...
rsync -av --delete \
  ...
  -e "ssh -p 22" \
  ./webapp/ root@132.232.152.250:/opt/medical-audit/webapp/

# 重启
ssh root@132.232.152.250 'systemctl restart medical-audit.service'
...
服务器: 腾讯云 CVM `132.232.152.250:22`,服务 `medical-audit.service`,目录 `/opt/medical-audit/webapp/`。

`kp.db` 体积约 434 MB,所以代码与数据走两条独立的同步路径。

### 本机同步 (有外网时)

```powershell
# 默认:同步所有小文件 + kp.db + 重启服务 + 健康检查 (5-15 分钟)
.\scripts\sync_to_cvm.ps1

# 只同步小文件,数据库保持远端不动
.\scripts\sync_to_cvm.ps1 -SkipDb
```

实现:`scripts/sync_to_cvm.ps1` → `scripts/_ssh.py` (Paramiko SFTP)。凭据硬编码默认值是项目私有的,生产建议通过 `-SshPass` 覆盖或环境变量 `MA_SSH_PASS`。

依赖:`pip install paramiko`。

### GitHub Actions 自动同步 (推荐)

`push to main` 影响 `webapp/**` 时自动触发 (不含 `webapp/data/kp.db`,见 .gitignore):

- `.github/workflows/deploy.yml` — `appleboy/scp-action` 推代码,`appleboy/ssh-action` 重启 + `curl` 健康检查
- `.github/workflows/sync-db.yml` — **手动触发** (`workflow_dispatch`),上传并原子替换 `kp.db`

需要在仓库 `Settings -> Secrets and variables -> Actions` 配置三个 Secret:

| Secret | 值 |
|---|---|
| `SSH_HOST` | `132.232.152.250` |
| `SSH_USER` | `root` |
...
```powershell
# 远端 DB 备份按 kp.db.bak.YYYYMMDD_HHMMSS 命名,任选一个:
ssh root@132.232.152.250 "cd /opt/medical-audit/webapp/data && mv kp.db kp.db.failed && mv kp.db.bak.20260101_120000 kp.db && systemctl restart medical-audit.service"
```
