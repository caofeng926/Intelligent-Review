...
| 表 | 数量 | 说明 |
|---|---|---|
| `batches` | 21 | 批次 (含 NHSA + PDF 2025,截至 2026-07-27 第十九/二十批入库后) |
...
- **服务器**：腾讯云 CVM `ubuntu@132.232.152.250:2222` (SSH `ubuntu` / 端口 2222 / 重启 systemctl 需 `echo "$MA_SSH_PASS" | sudo -S` 喂密码,因 ubuntu 无 sudoers / 凭据走 `$MA_SSH_PASS` 环境变量)
...
- **远程同步** (Paramiko / scp)：
  ...
  ssh.connect("132.232.152.250", port=2222, username="ubuntu", password=os.environ["MA_SSH_PASS"], timeout=15)
  sftp = ssh.open_sftp()
